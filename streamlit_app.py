...
# ===================== SAYFA 2: Gerilim Düşümü — Gerçek Veri & AI =====================
    ...
    # ------- Eğitim verisi (ext_df yoksa sentetik, %15 clip) -------
    def build_training_df(ext_df):
        ...
        k_vals = np.clip(
            rng.normal(loc=k_const if k_const > 0 else 1e-4,
                       scale=0.25 * (k_const if k_const > 0 else 1e-4), size=n),
            1e-6, 1.0
        )
    ...
    dv_ai = float(reg.predict(pd.DataFrame([{"L_m": L_in, "P_kw": N_in, "k": k_in}]))[0]) if reg is not None else float("nan")
    dv_formula = float(np.clip(dv_formula, 0, 15)) if np.isfinite(dv_formula) else dv_formula
    dv_ai = float(np.clip(dv_ai, 0, 15)) if np.isfinite(dv_ai) else dv_ai
    ...
    dloc = dloc.sort_values("Mesafe (m)").head(5).reset_index(drop=True)

    rng = np.random.default_rng(42)
    if "Yük (kW)" in dloc.columns:
        dloc["Yük (kW)"] = pd.to_numeric(dloc["Yük (kW)"], errors="coerce").fillna(rng.integers(10, 300, size=len(dloc)))
    else:
        dloc["Yük (kW)"] = rng.integers(10, 300, size=len(dloc))

    dloc["Gerçek (%)"] = dloc.apply(lambda r: vdrop_kLN(r["Mesafe (m)"], r["Yük (kW)"], k_in), axis=1)
    if reg is not None:
        Xb = dloc[["Mesafe (m)", "Yük (kW)"]].copy(); Xb["k"] = k_in
    ...
    fig_cmp = px.line(
        plot_df, x="Direk", y=["Gerçek (%)", "Tahmin (%)"], markers=True,
        template="plotly_white", title=f"{trafo_sel} — Gerçek vs AI"
    )
